...
export function normalizeActionMarketAcceptedEntry(entry, {
  fallbackDay = 1,
  category
} = {}) {
  ...
  const templateCategory = (() => {
    if (typeof entry.templateCategory === 'string' && entry.templateCategory.trim().length) {
      return entry.templateCategory.trim();
    }
...
export function createAcceptedEntryFromOffer(offer, details = {}, {
  fallbackDay = 1,
  category
} = {}) {
  if (!offer) {
  ...
  return normalizeActionMarketAcceptedEntry({
    offerId: offer.id,
    ...
    seats: offer.seats,
    templateCategory: offer.templateCategory ?? category
