...
export function getCachedHustleModel(id) {
  return hustleModelCache.get(id) || null;
}

export function hasHustleCard(id) {
  return hustleUi.has(id);
}
...
export function renderHustles(definitions, hustleModels = []) {
  ...
  container.innerHTML = '';
  hustleUi.clear();
  const modelMap = buildModelIndex(hustleModels);
...
export function updateHustleCard(definition, model, { emitEvent } = {}) {
  ...
  if (availabilityChanged) {
    emitEvent?.('hustles:availability-updated');
  }
}
