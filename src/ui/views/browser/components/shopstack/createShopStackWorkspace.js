import { registerAssetWorkspace } from '../../utils/assetWorkspaceRegistry.js';
import {
  initialState,
...
let presenter;
let activeDefinitionMap = new Map();
...
const shopStackWorkspaceRegistration = registerAssetWorkspace({
  assetType: 'shopstack',
  className: 'shopstack',
  defaultView: VIEW_CATALOG,
  state: { ...initialState },
  ensureSelection(state, model) {
    ensureCatalogSelection(state, model, activeDefinitionMap);
  },
  deriveSummary: model => deriveWorkspaceSummary(model, activeDefinitionMap),
  derivePath(state, model) {
    return deriveShopStackPath(state, model, activeDefinitionMap);
  },
  isLocked: () => false,
  ...
  views: [
    {
      ...
      label: 'Catalog',
      badge: ({ model, state }) => {
        const items = collectCatalogItems(model, activeDefinitionMap);
      ...
      render(context) {
        const handlers = createCatalogHandlers({ presenter, definitionMap: activeDefinitionMap });
        return renderCatalogView({
          model: context.model,
          state: context.state,
          definitionMap: activeDefinitionMap,
          handlers: {
            ...handlers,
            onBuy: handleBuy
          }
...
export function createShopStackWorkspacePresenter() {
  activeDefinitionMap = new Map();
  presenter = shopStackWorkspaceRegistration.createPresenter();

  const originalRender = presenter.render.bind(presenter);
  presenter.render = (model = {}, context = {}) => {
    if (Array.isArray(context.definitions)) {
      activeDefinitionMap = createDefinitionMap(context.definitions);
    }
