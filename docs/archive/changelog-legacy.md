...
This archive preserves the detailed notes that previously lived in `docs/changelog.md`. For the current summary, see `../changelog.md`. The sections below remain exactly as they were authored (including the legacy `Unreleased` bucket) to keep historical context intact.
